...
ohlcv_histories, technical_indicators, next_day_open_values, unscaled_y, y_normaliser = csv_to_dataset('data/%s.csv' % sys.argv[1])
...
z = Dense(64, activation="sigmoid", name='dense_pooling')(technical_indicators_branch.output)
...
model = Model(inputs=[technical_indicators_branch.input], outputs=z)
adam = optimizers.Adam(lr=0.0005)
model.compile(optimizer=adam, loss='mse')
# model.fit(x=[ohlcv_train, tech_ind_train], y=y_train, batch_size=32, epochs=50, shuffle=True, validation_split=0.1)
model.fit(x=[tech_ind_train], y=y_train, batch_size=32, epochs=50, shuffle=True, validation_split=0.1)
...
y_predicted = model.predict([technical_indicators])
...
print(f'Mean Squared Error for {sys.argv[1]} is: {scaled_mse}')
